...
function TOKENIZER_FUNCS:next_token()
	...
	elseif c == C_QUOTE then
		return { type = "string", value = self:read_quoted_string() }
	else
		return { type = "string", value = self:read_unquoted_string() }
	end
end
	...
	if tok.type == "=" then
		...
		if tok.type == "string" then
			...
			self:report_error(
				...
				string.format("parsing attribute '%s' - expected a string after =, but did not find one"),
				name
			)
		end
	else
